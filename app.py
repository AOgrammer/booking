...
page = st.sidebar.selectbox('ページを選択', ['ユーザー登録', '会議室登録', '予約登録', 'ユーザー更新・削除', '会議室更新・削除', '予約更新・削除'])
...
if page == 'ユーザー登録':
    ...
    st.title('会議室予約画面')
    url_users = 'http://0.0.0.0:8000//users'
    res = requests.get(url_users)
    users = res.json()
    users_name = {user['username']: user['user_id'] for user in users}

    url_rooms = 'http://0.0.0.0:8000//rooms'
    ...
    url_bookings = 'http://0.0.0.0:8000//bookings'
    ...
    if submit_button:
        ...
        if booked_num > capacity:
            ...
            url = 'http://0.0.0.0:8000//bookings'
            res = requests.post(url, data=json.dumps(data))
            if res.status_code == 200:
                st.success('予約完了しました')
    ...
    if update_button:
        url = f'http://0.0.0.0:8000//users/{user_id}'
    ...
    if update_button:
        url = f'http://0.0.0.0:8000//rooms/{room_id}'
    ...
    if update_button:
        ...
        if booked_num > capacity:
            ...
            url = f'http://0.0.0.0:8000//bookings/{booking_id}'
